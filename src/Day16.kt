...
//            return when (typeId) {
//                4 -> return value!!
//                0 -> return children.sumOf { it.getValue() }
//                1 ->
//                    children.map { it.getValue() }.reduce { acc, i -> acc * i }
//                2 -> {
//                    return children.minOf { it.getValue() }
//                }
//                3 -> {
//                    return children.maxOf { it.getValue() }
//                }
...
//        }
    ...
    fun parsePacket(pPacket: Packet, counter: Int, bs: String) {
        ...
        if (typeId == literalCode) {
            ...
            packet.setValue(literal)
            pPacket.addChild(packet)
            ...
            if (bs[offset++] == '0') {
                ...
                offset += 11
                val r = pPacket.addChild(packet)
                if (r)
                    parsePacket(packet, offset, bs)
                else {
                    packet.setLimit(lm)
                    parsePacket(packet, offset, bs)
                }
